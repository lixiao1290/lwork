...
class Server
{
    ...
    /**
     * This returns the UID of messages that the index position.
     *
     * @return int
     */
    public function getUidByPositon($position)
    {
        return imap_uid($this->getImapStream(), $position);
    }

    /**
     * This returns the position from UID message.
     *
     * @return int
     */
    public function getPositonByUid($uid)
    {
    ...
    public function getMessages($limit = null, $offset = null, $order = null, $typeOrder = 'asc')
    {
        ...
        $negative = false;
        // check limit if it is negative
        if($limit < 0)
        {
            $negative = true;

            // limit is negative value
        ...
        $nMessages  = $this->numMessages();

        // check that there are message
        if ($nMessages < 1)
            return [];
        ...
        if($order != null)
            $msgIds = imap_sort($this->getImapStream(), $order, $typeOrder == "asc" || $typeOrder == "ASC" ? 1 : 0, SE_UID);
        ...
        for ($i = 0; $i < $limit; $i++)
        {
            if($order == null)
            {
                $uid        = imap_uid($stream, $index);
                $messages[] = new Message($uid, $this);
            }
            else
            {
                $messages[] = new Message($msgIds[$index-1], $this);
            }
